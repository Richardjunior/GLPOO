...
public class Julia extends JComponent {
	...
	public static int ITERATIONS = 100; // Définition du nombre d'itérations
	public static float ECHELLE = 500; // Definition de l'echelle de grandeur
	...
	public Julia(int iterations, float echelle) {
		buffer = new BufferedImage(LARGEUR, HAUTEUR, BufferedImage.TYPE_INT_RGB); // On instancie le buffer : dimensions
																					// et coloration
	...
	public int calculCouleur(float zx, float zy) {
		...
		for (; n < ITERATIONS; n++) {
			if (zx * zx + zy * zy > 16)
				break; // Condition "infinie" dans notre monde fini
		...
		if (n == ITERATIONS)
			return 0x00000000; // blanc => hors de l'ensemble
