...
public class Main extends JComponent
{
	...
	public static final int ITERATIONS = 5000; // Définition du nombre d'itérations
	...
	public static final Complex r2 = new Complex(-1/2,-Math.sqrt(3)/2); // Racine complexe du polynome de Newton classique
	public static final Complex r3 = new Complex(-1/2,Math.sqrt(3)/2); // Racine complexe conjuguée
	
	public static final float epsilon = (float)0.5; //plus epsilon est petit, plus le calcul est précis, et le temps de calcul évolue de manière exponentielle. Tester avec de petites valeurs au début
				...
				if(Math.abs(z.getReal()-item.getReal()) < epsilon && Math.abs(z.getImaginary()- item.getImaginary()) < epsilon)
						{
							...
							{
								return Color.HSBtoRGB((float)Math.sqrt((double)300*n/ITERATIONS) , 0.5f, 1); // ce point converge vers la racine réelle
							}
							else if(item.getImaginary()<0)
							{
								return Color.HSBtoRGB((float)Math.sqrt((double)300*n/ITERATIONS) , 0.5f, 1); // ce point converge vers la 1ere racine complexe
							}
							else if(item.getImaginary()>0)
							{
								return Color.HSBtoRGB((float)Math.sqrt((double)300*n/ITERATIONS) , 0.5f, 1); // ce point converge vers la racine complexe conjuguée
							}
						}
			}
		}
